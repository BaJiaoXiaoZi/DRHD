...
if select_file:
    ...
    with tab0:
        col1_1, col1_2 = st.columns((1.9,2))
        col1_1.markdown(
            "<h4 style='text-align: left; black: gray;'>📕The Diabetic Retinopathy-related Homeostatic Dysregulation (DRHD) value was derived based on the selected DR-related biomarkers! </h2>",
            unsafe_allow_html=True)
        col1_1.markdown(
            "<h4 style='text-align: left; black: gray;'>the construction of the DRHD value relied upon a reference group of participants devoid of any retinopathy, as confirmed by retinal imaging assessments. It should be acknowledged that alterations to the reference group could potentially impact the DRHD value. </h2>",
            unsafe_allow_html=True)
        col1_1.markdown(
            "<h4 style='text-align: left; black: gray;'>It demonstrated an independent and linear dose-response association with increased risk of mortality from all causes, CVD and diabetes.</h2>",
            unsafe_allow_html=True)
        col1_1.markdown("<h3 style='text-align: left; black: gray;'>🔎Read more in our paper⭐.</h2>",
            unsafe_allow_html=True)
        image3 = Image.open('OR.png')
        col1_2.image(image3, caption=None, width=500, use_column_width=False, clamp=False, channels='RGB')
    ...
    with tab2:
        ...
        with col5_1:
            st.write('#### :black[①This is the DRHD of your data(We add it to the last two columns)]')
            a, b = md_calc(df)
            df['HD'] = a
            df['HD_log'] = b
        ...
        with col5_2:
            ...
            st.write('')
            st.write('')
            st.image(image2, caption=None, width=800, use_column_width=False, clamp=False, channels='RGB')
            # st.write('***')
        st.markdown('---')
